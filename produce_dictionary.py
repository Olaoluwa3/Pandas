...
#1. Produce that had the highest and lowest sales in total sales (both name of produce and value)
print("-------------------------------- 1 -----------------------------")
##sortedDF = df.T.sort_values('Total Sale', ascending=True)
minTotalSale = df.T['Total Sale'].min()
maxTotalSale = df.T['Total Sale'].max()

for col_name in df:
    if df.loc['Total Sale', col_name] == minTotalSale:
        print(f"Produce name: {col_name}, Minimum Value: {minTotalSale}")
    if df.loc['Total Sale', col_name] == maxTotalSale:
        print(f"Produce name: {col_name}, Maximum Value: {maxTotalSale}")


####Another Way###  - for personal knowledge
name_min = df.T['Total Sale'].idxmin()
name_max = df.T['Total Sale'].idxmax()
...
#6. Create a new dataframe for only those produce that have sold between 11,500 to 12,000 (quantity)
print("-------------------------------- 6 -----------------------------")
newDF = (df.T[df.T['Quantity Sold'].between(11500,12000)]).T
print(newDF)


#7. What is the total sales for the products in the above new dataframe? (print out ONLY total sales)
print("-------------------------------- 7 -----------------------------")
print(newDF.T['Total Sale'].sum())